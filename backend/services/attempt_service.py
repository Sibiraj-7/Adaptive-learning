    ...
    if assignment.get("target_type") == "department":
        def _norm(v: str) -> str:
            ...
            if not raw:
                return ""
            words = raw.split()
            return raw.upper() if len(words) == 1 else "".join(w[0].upper() for w in words)
        return _norm(student.get("department")) == _norm(assignment.get("department"))
...
def submit_attempt(student_id: str, payload: dict) -> dict[str, Any]:
    ...
    for qoid in qids_order:
        ...
        lst = subjects_topics.setdefault(subject, [])
        if topic not in lst:
            lst.append(topic)
    ...
    quiz_difficulty = (quiz.get("difficulty") or "medium").strip().lower()
    ...
    if existing_mastery_doc:
        prev_val = (existing_mastery_doc.get("topic_mastery") or {}).get(current_topic)
    ...
    for subj, unique_topics in subjects_topics.items():
        ...
        prev_map: dict[str, float] = dict((existing or {}).get("topic_mastery") or {})
        ...
        for topic in unique_topics:
            ...
            prev_map[topic] = _blend_mastery(
                prev_map.get(topic) if topic in prev_map else None,
                new_frac,
            )
    ...
    all_done = recommendation.get("all_quizzes_completed", False)
    ...
    return {
        ...
        "mastery_level":          recommendation.get("mastery_level", ""),
        "recommendation_message": (
            "You have completed all assigned quizzes! Great work."
            if all_done
            else f"Next recommended topic: {recommendation['recommended_next_topic']}"
        ),
    }