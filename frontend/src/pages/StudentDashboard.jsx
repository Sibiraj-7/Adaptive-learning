...
export default function StudentDashboard() {
  ...
  const actionMeta = {
    revisit:  { icon: '🔁', label: 'Revisit',  color: 'text-rose-700',    bg: 'bg-rose-50',    border: 'border-rose-200' },
    practice: { icon: '📝', label: 'Practice', color: 'text-amber-700',   bg: 'bg-amber-50',   border: 'border-amber-200' },
    advance:  { icon: '🚀', label: 'Advance',  color: 'text-emerald-700', bg: 'bg-emerald-50', border: 'border-emerald-200' },
  }
  ...
  const getLatestAttemptForQuiz = (quizId, assignmentId) => {
    ...
    return attempts.find(
      (a) => String(a.quiz_id) === String(quizId) && String(a.assignment_id) === String(assignmentId)
    ) || null
  }
  ...
  const bestByTopicDifficulty = useMemo(() => {
    const map = {}
    const getTime = (v) => { const ms = Date.parse(v); return Number.isFinite(ms) ? ms : 0 }
    for (const row of assigned || []) {
      ...
      if (!currentBest || getTime(q.created_at) >= getTime(currentBest?.quiz?.created_at)) {
  ...
  const learningPath = useMemo(() => {
    const mapped = topics.map((topic) => {
      ...
      return { topic, stagesCompleted, mastered }
    }).sort((a, b) => {
    ...
    return mapped.map((item) => {
      if (!item.mastered && !pinAssigned && item.stagesCompleted > 0) {
        pinAssigned = true
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-2xl border border-indigo-200 bg-gradient-to-br from-indigo-50 to-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-xl font-semibold text-slate-900">{rec ? formatTopic(rec) : '—'}</p>
      </div>

      {aiDecision && sInfo && aInfo && (
        <div className={`rounded-2xl border p-6 shadow-sm ${sInfo.border} ${sInfo.bg}`}>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div className="rounded-xl bg-white/70 p-3 text-center shadow-sm">
              ...
              <p className={`text-sm font-bold capitalize ${sInfo.color}`}>{aiDecision.masteryLevel || '—'}</p>
            </div>
            <div className="rounded-xl bg-white/70 p-3 text-center shadow-sm">
            ...
            <div className="rounded-xl bg-white/70 p-3 text-center shadow-sm">
              ...
              <p className={`text-sm font-bold ${aInfo.color}`}>{aInfo.icon} {aInfo.label}</p>
            </div>
            <div className="rounded-xl bg-white/70 p-3 text-center shadow-sm">
          ...
          <p className="mt-3 text-sm text-slate-600 italic">💡 {sInfo.desc}</p>
        </div>
      )}
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500">Topics ordered by your progress — focus on the highlighted one first.</p>
            ...
            {learningPath.map((item, idx) => (
              <div key={item.topic} className="flex items-center gap-2">
                ...
                >
                  ...
                  {item.topic}
                  <span className="ml-2 text-xs opacity-75">{item.stagesCompleted}/3</span>
                </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-slate-900">Learning Path Progress</h2>
        {topics.length === 0 ? (
              ...
              return (
                <div key={topic} className="rounded-xl border border-slate-200 bg-white p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-sm font-semibold text-slate-900">{topic}</h3>
                    <span className="text-xs text-slate-500">{stagesCompleted}/3 stages</span>
                  </div>
                    ...
                    {diffOrder.map((d) => (
                      <div
                        ...
                        className={`h-2 flex-1 rounded-full transition-all
                          ${completion[d] ? 'bg-emerald-500' : canStart(completion, d) ? 'bg-indigo-300' : 'bg-slate-100'}`}
                      />
                    ))}
                  </div>
                  <div className="space-y-2">
                    {diffOrder.map((difficulty) => {
                      ...
                      const latestAttemptForQuiz = hasQuiz ? getLatestAttemptForQuiz(quizId, assignmentId) : null
                      const attemptPct = getAttemptPercentage(latestAttemptForQuiz)
                      ...
                      return (
                        <div key={difficulty} className="flex items-center justify-between gap-3">
                          ...
                          {isPassed ? (
                            ...
                            <span className="rounded-xl bg-slate-100 px-4 py-1.5 text-xs font-semibold text-slate-500 ring-1 ring-slate-200">Locked</span>
                          ) : !hasQuiz ? (
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {weak.length === 0 ? (
            <p className="mt-3 text-sm text-slate-500">Complete a quiz to see mastery estimates.</p>
          ) : (
            <ul className="mt-4 space-y-3">
              {weak.slice(0, 10).map((w) => (
                <li key={w.key} className="flex items-center justify-between gap-3 rounded-lg bg-slate-50 px-3 py-2 text-sm">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {(dash?.suggested_materials || []).length === 0 ? (
            <p className="mt-3 text-sm text-slate-500">No materials in the database for this topic yet.</p>
          ) : (
            <ul className="mt-4 space-y-3">
              {(dash?.suggested_materials || []).map((m, mi) => (
                <li key={`${m.title}-${mi}`}>
                  <a href={m.url} target="_blank" rel="noreferrer" className="font-medium text-indigo-600 hover:text-indigo-800 hover:underline">
                    {m.title}
                  </a>
                  {m.topic && <p className="text-xs text-slate-500">{formatTopic(m.topic)}</p>}
                </li>
              ))}
            </ul>
          )}
